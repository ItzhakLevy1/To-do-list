...
let port = process.env.PORT;  // Allowing our program to run both on our local system as well as on heroku.
if (port == null || port == "") {   // Check if a port from heroku exists, if not than run the program localy on port 3000. 
  port = 3000;
}
app.listen(port);

app.listen(port, function() {   // Referencing "port" that could be either heroku's port "process.env.PORT" or our local port 3000.
